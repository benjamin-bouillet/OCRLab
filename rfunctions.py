...
def rloadsave():
	"""Fonction de chargement des sauvegardes. Si le fichier 'roboc_save_file' n'existe pas, il est créé vide"""
...
def rdumpsave(par,rsave,cmap):
	"""Fonction de vidage de la partie dans un fichier de sauvegarde 'roboc_save_file'"""
	if cmap!=None:
		rsave[cmap]=par
...
def rchoicemap():
	"""Fonction de choix de la carte parmi les cartes présentes dans le dossier 'cartes'"""
...
def rchecksave(chosen_map, rsave, roboc_user):
	"""Fonction de vérification de la présence d'une sauvegarde sur une map donnée et dans un fichier de sauvegarde donnée. Prend comme argument une carte (classe 'Rmap') dans un dictionnaire de sauvegarde rsave. La fonction renvoie une partie par (class rgame)"""

	# Verification de la présence d'une sauvegarde sur la carte
	if chosen_map in rsave:
		print("\nLe joueur",rsave[chosen_map]._player,"a déjà une partie en cours pour la carte",chosen_map,'.')
...
def rinputchoice_net(connection_with_client):
	...
	error_case=str("Merci de rentrer une action conforme :\n- N/S/E/O pour vous deplacer d\'une case,\n- N/S/E/O+X pour vous deplacer de X cases (ex: N3 ou O2)\n- Q pour quitter\n")
