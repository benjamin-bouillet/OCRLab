...
class Rwall(Rsquare):
    ...
    def mouv(self, coord, action):
        ...
        # Renvoie une validation d'action conforme, les nouvelles coordonnées
        # du joueur ainsi qu'une absence de message (un déplacement sur une
        # case vide ne génère aucun message d'alerte au joueur)
...
class Rdoor(Rsquare):
    ...
    def __init__(self):
        """Constructeur de la classe ; permet la gestion du murage et
        les cas de représentation graphique"""
    ...
    def mouv(self, coord, action):
        ...
        if self._iswalled:
            confaction = False
            retcoord = coord
            message = "Il vous faut percer ce mur\
                      avant de pouvoir le traverser\n"
        else:
            confaction = True
            (i, j) = coord
            if action == 'N':
    ...
    def wall(self):
        ...
        if self._iswalled:
            confaction = False
            message = "Cette porte est déjà murée, pourquoi ne pas essayer\
                      de la percer ?\n"
        else:
            self._iswalled = True
            confaction = True
...
class Rplayer(Rsquare):
    ...
    def mouv(self, coord, action):
        ...
        return (False,
                coord,
                "Ne marchez pas sur les pieds de votre adversaire !\n"
                )
